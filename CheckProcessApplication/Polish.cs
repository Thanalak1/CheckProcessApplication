    ...
    public partial class Polish : Form
    {
        ...
        private void Search()
        {
            ...
            Center.cmd.CommandText = $@"WITH PolishData AS
                                        ...
                                        CASE WHEN SUM(ROUND(((((ISNULL(AccEmp2.OKWG, 0)) * ISNULL(JobDetail.DMPercent, 0)) / 100) * 100) / 100, 2)) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) = 0 THEN '' ELSE CASE WHEN SUM(ISNULL(AccEmp2.OKWG, 0)) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) + ISNULL(JobBills.RtWg, 0) + ISNULL(JobBills.Silver, 0) + ISNULL(V_JobMaterial_Sum.MatRecWg, 0) + SUM(ROUND(((((ISNULL(AccEmp2.OKWG, 0)) * ISNULL(JobDetail.DMPercent, 0)) / 100) * 100) / 100, 2)) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) - (SUM(JobDetail.Ttlwg) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) + ISNULL(V_JobMaterial_Sum.MatRecWg, 0) + ISNULL(CTE_JobGemDetail.GemWg, 0)) > SUM(ROUND(((((ISNULL(AccEmp2.OKWG, 0)) * ISNULL(JobDetail.DMPercent, 0)) / 100) * 100) / 100, 2)) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) THEN 'ไม่ถูกต้อง' ELSE '' END END AS DiffWg,
                                        CASE WHEN CAST((ISNULL(AccEmp.EmpAmount, 0) + ISNULL(AccEmp.ISAAmount, 0)) AS DECIMAL(18, 2)) <> CAST(SUM(ISNULL(AccEmp2.AccPrice, 0) * CASE WHEN AccEmp2.OKQty = 0.5 THEN 0.5 ELSE CASE WHEN TEmpProfile.TitleName = 'บริษัท' THEN AccEmp2.OKQty ELSE FLOOR(AccEmp2.OKQty) END END) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) - (AccEmp.DMWG + AccEmp.LSWG + AccEmp.DMGemAmount + AccEmp.LSGemAmount + AccEmp.DeductAmount + AccEmp.ISAAmount) AS DECIMAL(18, 2)) THEN 'ค่าแรงไม่ถูกต้อง' ELSE '' END AS Wage,
                                        ...
                                        SELECT DENSE_RANK() OVER (ORDER BY DocNo) AS [ลำดับที่], EmpCode, EmpName, Inv_No, DocNo, JobBarCode, JobDate, TypeOfWork, TypeOfBill, Late, Less, LateFee, CenterAcc, DeductLost, DiffWg, Wage, (SELECT SUM(RawDiffWg1) FROM PolishData) AS DiffWg1, (SELECT SUM(RawDiffMat1) FROM PolishData) AS DiffMat1, DiffWg2, DiffMat2, (SELECT SUM(DiffTWg) FROM (SELECT DISTINCT DocNo, DiffTWg FROM PolishData) AS DistinctDiffTWg) AS DiffTTWg, (SELECT SUM(RawPerDiffTTWg) FROM (SELECT DISTINCT DocNo, RawPerDiffTTWg FROM PolishData) AS DistinctDiffTWg) AS PerDiffTTWg, (SELECT SUM(NormalEmpAmount) FROM (SELECT DISTINCT DocNo, NormalEmpAmount FROM PolishData) AS DistinctData) AS NetWage FROM PolishData ORDER BY JobBarCode;";
