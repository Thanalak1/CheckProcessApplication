    ...
    public partial class Polish : Form
    {
        ...
        private void Search()
        {
            ...
            Center.cmd.CommandText = $@"WITH PolishData AS
                                        ...
                                        CASE WHEN ISNULL(AccEmp.EmpAmount, 0) + ISNULL(AccEmp.ISAAmount, 0) <> ISNULL(JobDetail.PriceJob, 0) * ISNULL(SUM(CASE WHEN AccEmp2.OKQty = 0.5 THEN 0.5 ELSE CASE WHEN TEmpProfile.TitleName = 'บริษัท' THEN AccEmp2.OKQty ELSE FLOOR(AccEmp2.OKQty) END END) OVER (PARTITION BY AccEmp2.DocNo, AccEmp2.EmpCode), 0) - (ISNULL(AccEmp.DMWG, 0) + ISNULL(AccEmp.LSWG, 0) + ISNULL(AccEmp.DMGemAmount, 0) + ISNULL(AccEmp.LSGemAmount, 0) + ISNULL(AccEmp.DeductAmount, 0))THEN 'ค่าแรงไม่ถูกต้อง' ELSE '' END AS Wage,
