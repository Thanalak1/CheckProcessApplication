    ...
    public partial class Polish : Form
    {
        ...
        private void Search()
        {
            ...
            Center.cmd.CommandText = $@"WITH PolishData AS
                                        ...
                                        CASE WHEN CAST(ISNULL(AccEmp2.OKWG, 0) + ISNULL(V_JobMaterial_NoSum.MatRecWg, 0) + ISNULL(JobBills.RtWg, 0) + ISNULL(JobBills.Silver, 0) + FLOOR((ISNULL(AccEmp2.OKWG, 0) - (ISNULL(JobDetail.MatWg, 0) + ISNULL(JobDetail.MatWg2, 0))) * ISNULL(JobDetail.DMPercent, 0) / 100) - ISNULL(JobDetail.TtlWg, 0) + ISNULL(JobDetail.MatWg, 0) + ISNULL(JobDetail.MatWg2, 0) AS DECIMAL(18, 2)) > CAST((ISNULL(AccEmp2.OKWG, 0) - ISNULL(V_JobMaterial_Sum.MatWg, 0) + ISNULL(V_JobMaterial_Sum.MatWg2, 0)) / 100.0 AS DECIMAL(18, 2)) THEN 'ไม่ถูกต้อง' ELSE '' END AS DiffWg,
                                        ...
                                        --CASE WHEN ISNULL(AccEmp.EmpAmount, 0) + ISNULL(AccEmp.ISAAmount, 0) <> (ISNULL(JobDetail.PriceJob, 0) * FLOOR(ISNULL(CASE WHEN AccEmp2.OKQty = 0.5 THEN 0.5 ELSE AccEmp2.OKQty END, 0)) - (ISNULL(AccEmp.DMWG, 0) + ISNULL(AccEmp.LSWG, 0) + ISNULL(AccEmp.DMGemAmount, 0) + ISNULL(AccEmp.LSGemAmount, 0) + ISNULL(AccEmp.DeductAmount, 0))) THEN 'ค่าแรงไม่ถูกต้อง' ELSE '' END AS Wage,
                                        
                                        CASE WHEN ISNULL(AccEmp.EmpAmount, 0) + ISNULL(AccEmp.ISAAmount, 0) <>
(CASE WHEN AccEmp2.RTQty = 0.5 THEN ISNULL(JobDetail.PriceJob, 0) * FLOOR(ISNULL(SUM(CASE WHEN AccEmp2.OKQty = 0.5 THEN 0.5 ELSE AccEmp2.OKQty END) OVER (PARTITION BY AccEmp2.DocNo, AccEmp2.EmpCode), 0)) ELSE ISNULL(JobDetail.PriceJob, 0) * ISNULL(SUM(CASE WHEN AccEmp2.OKQty = 0.5 THEN 0.5 ELSE AccEmp2.OKQty END) OVER (PARTITION BY AccEmp2.DocNo, AccEmp2.EmpCode), 0) END)
                                        - (ISNULL(AccEmp.DMWG, 0) + ISNULL(AccEmp.LSWG, 0) + ISNULL(AccEmp.DMGemAmount, 0) + ISNULL(AccEmp.LSGemAmount, 0) + ISNULL(AccEmp.DeductAmount, 0))THEN 'ค่าแรงไม่ถูกต้อง' ELSE '' END AS Wage,
                                        ...
                                        --SUM((ISNULL(JobDetail.PriceJob, 0) * (CASE WHEN ISNULL(AccEmp2.OKQTY, 0) = 0.5 THEN ISNULL(AccEmp2.OKQTY, 0) ELSE CASE WHEN ISNULL(AccEmp2.OKQTY, 0) > 0.5 THEN FLOOR(ISNULL(AccEmp2.OKQTY, 0)) ELSE 0 END END)) - (ISNULL(AccEmp.DMWG, 0) + ISNULL(AccEmp.LSWG, 0) + ISNULL(AccEmp.DMGemAmount, 0) + ISNULL(AccEmp.LSGemAmount, 0) + ISNULL(AccEmp.DeductAmount, 0))) OVER () AS NetWage
                                        --SUM((ISNULL(JobDetail.PriceJob, 0) * (CASE WHEN ISNULL(AccEmp2.OKQTY, 0) = 0.5 THEN 0.5 ELSE CASE WHEN ISNULL(AccEmp2.OKQTY, 0) > 0.5 THEN ISNULL(AccEmp2.OKQTY, 0) ELSE 0 END END)) - (isnull(AccEmp.ISAAmount, 0) + isnull(AccEmp.DMGemAmount, 0) + isnull(AccEmp.DMWG, 0) + isnull(AccEmp.LSWG, 0) + isnull(AccEmp.GemRepeat, 0) + isnull(AccEmp.DeductAmount, 0) + isnull(AccEmp.LSGemAmount, 0))) OVER () AS NetWage
                                        SUM((ISNULL(JobDetail.PriceJob, 0) * ISNULL(AccEmp2.OKQTY, 0)) - (ISNULL(AccEmp.DMWG, 0) + ISNULL(AccEmp.LSWG, 0) + ISNULL(AccEmp.DMGemAmount, 0) + ISNULL(AccEmp.LSGemAmount, 0) + ISNULL(AccEmp.DeductAmount, 0))) OVER () AS NetWage
