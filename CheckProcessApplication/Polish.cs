    ...
    public partial class Polish : Form
    {
        ...
        private void Search()
        {
            ...
            Center.cmd.CommandText = $@"WITH RankedData AS
                                        ...
                                        CASE WHEN EXISTS (SELECT 1 FROM JobKeep WHERE JobKeep.DocNo = AccEmp.DocNo AND JobKeep.TtQty < 0.7 * JobDetail.TtQty AND JobKeep.mDate < DATEADD(DAY, 13, JobHead.JobDate) AND JobKeep.Num = 1) THEN 'YES' ELSE '' END AS Less,
                                        CASE WHEN (SELECT MIN(JobKeep.mDate) FROM JobKeep WHERE JobKeep.DocNo = AccEmp.DocNo) > DATEADD(DAY, 10, JobHead.JobDate) OR EXISTS (SELECT 1 FROM JobKeep WHERE JobKeep.DocNo = AccEmp.DocNo AND JobKeep.TtQty < 0.7 * JobDetail.TtQty AND JobKeep.mDate < DATEADD(DAY, 13, JobHead.JobDate) AND JobKeep.Num = 1) THEN CAST((JobDetail.PriceJob * FLOOR(AccEmp2.OKQty)) / 2 AS DECIMAL(18, 2)) ELSE CAST(JobDetail.PriceJob * FLOOR(AccEmp2.OKQty) AS DECIMAL(18, 2)) END AS LateFee,
                                        ...
                                        CASE WHEN ISNULL(SUM(JobDetail.Ttlwg) OVER(), 0) - ( ISNULL(SUM(AccEmp.ISSWG) OVER(), 0) - ( ISNULL(SUM(V_JobMaterial_Sum.MatWg) OVER(), 0) + ISNULL(SUM(V_JobMaterial_Sum.MatWg2) OVER(), 0))) IS NOT NULL THEN CAST( ISNULL(SUM(JobDetail.Ttlwg) OVER(), 0) - ( ISNULL(SUM(AccEmp.ISSWG) OVER(), 0) - ( ISNULL(SUM(V_JobMaterial_Sum.MatWg) OVER(), 0) + ISNULL(SUM(V_JobMaterial_Sum.MatWg2) OVER(), 0))) AS DECIMAL(18, 2)) ELSE 0 END AS DiffWg1,
                                        CASE WHEN ISNULL(SUM(JobDetail.Matwg) OVER(), 0) + ISNULL(SUM(JobDetail.Matwg2) OVER(), 0) - (ISNULL(SUM(AccEmp.ISSWG) OVER(), 0) - (ISNULL(SUM(V_JobMaterial_Sum.MatWg) OVER(), 0) + ISNULL(SUM(V_JobMaterial_Sum.MatWg2) OVER(), 0))) IS NOT NULL THEN CAST(ISNULL(SUM(JobDetail.Matwg) OVER(), 0) + ISNULL(SUM(JobDetail.Matwg2) OVER(), 0) - (ISNULL(SUM(AccEmp.ISSWG) OVER(), 0) - (ISNULL(SUM(V_JobMaterial_Sum.MatWg) OVER(), 0) + ISNULL(SUM(V_JobMaterial_Sum.MatWg2) OVER(), 0)))AS DECIMAL(18, 2)) ELSE 0 END AS DiffMat1,
                                        CASE WHEN ISNULL(SUM(AccEmp2.OKWG) OVER(), 0) + ISNULL(SUM(JobBills.RtWg) OVER(), 0) + ISNULL(SUM(V_JobMaterial_Sum.MatRecWg) OVER(), 0) + ISNULL(SUM((AccEmp2.OKWG - (V_JobMaterial_Sum.MatWg + V_JobMaterial_Sum.MatWg2)) / 100) OVER(), 0) - ISNULL(SUM(AccEmp.OKWG) OVER(), 0) IS NOT NULL THEN CAST(ISNULL(SUM(AccEmp2.OKWG) OVER(), 0) + ISNULL(SUM(JobBills.RtWg) OVER(), 0) + ISNULL(SUM(V_JobMaterial_Sum.MatRecWg) OVER(), 0) + ISNULL(SUM((AccEmp2.OKWG - (V_JobMaterial_Sum.MatWg + V_JobMaterial_Sum.MatWg2)) / 100) OVER(), 0) - ISNULL(SUM(AccEmp.OKWG) OVER(), 0) AS DECIMAL(18, 2)) ELSE 0 END AS DiffWg2,
                                        CASE WHEN ISNULL(SUM(PSKTran2.recwg) OVER(), 0) - ISNULL(SUM(V_JobMaterial_Sum.MatRecWg) OVER(), 0) IS NOT NULL THEN CAST(ISNULL(SUM(PSKTran2.recwg) OVER(), 0) - ISNULL(SUM(V_JobMaterial_Sum.MatRecWg) OVER(), 0) AS DECIMAL(18, 2))ELSE 0 END AS DiffMat2,
                                        CASE WHEN ISNULL(SUM(AccEmp2.OKWG) OVER() + SUM(JobBills.RtWg) OVER() + SUM(V_JobMaterial_Sum.MatRecWg) OVER() + SUM((AccEmp2.OKWG - (V_JobMaterial_Sum.MatWg + V_JobMaterial_Sum.MatWg2)) / 100.0) OVER(), 0) - ISNULL(SUM(AccEmp.ISSWG) OVER(), 0) IS NOT NULL THEN CAST(ISNULL(SUM(AccEmp2.OKWG) OVER() + SUM(JobBills.RtWg) OVER() + SUM(V_JobMaterial_Sum.MatRecWg) OVER() + SUM((AccEmp2.OKWG - (V_JobMaterial_Sum.MatWg + V_JobMaterial_Sum.MatWg2)) / 100.0) OVER(), 0) - ISNULL(SUM(AccEmp.ISSWG) OVER(), 0) AS DECIMAL(18, 2)) ELSE 0 END AS DiffTTWg,
                                        CASE WHEN ISNULL(SUM(AccEmp.ISSWG) OVER(), 0) = 0 THEN 0 ELSE CAST((ISNULL(SUM(AccEmp2.OKWG) OVER(), 0) + ISNULL(SUM(JobBills.RtWg) OVER(), 0) + ISNULL(SUM(V_JobMaterial_Sum.MatRecWg) OVER(), 0) + ISNULL(SUM((AccEmp2.OKWG - (V_JobMaterial_Sum.MatWg + V_JobMaterial_Sum.MatWg2)) / 100.0) OVER(), 0) - ISNULL(SUM(AccEmp.ISSWG) OVER(), 0)) / CAST(ISNULL(SUM(AccEmp.ISSWG) OVER(), 0) AS DECIMAL(18, 2)) * 100 AS DECIMAL(18, 2)) END AS PerDiffTTWg,
	                                    ...
	                                    GROUP BY AccEmp.EmpCode, JobHead.EmpName, AccEmp.Inv_No, AccEmp.DocNo, AccEmp2.JobBarCode, JobHead.JobDate, LEFT(JobDetail.Article, 1), CProfile.TdesArt, JobHead.ChkReturn, JobHead.ChkAccount, JobDetail.PriceJob, AccEmp2.OKQty, JobDetail.TtQty, JobCost.Cost3, AccEmp2.OKWG, V_JobMaterial_Sum.MatWg, V_JobMaterial_Sum.MatWg2, AccEmp.DMWG, AccEmp.LSWG, AccEmp.DMGemAmount, AccEmp.LSGemAmount, AccEmp.DeductAmount, AccEmp.EmpAmount, AccEmp.ISAAmount, AccEmp.ISSWG, JobDetail.Ttlwg, JobDetail.Matwg, JobDetail.Matwg2, AccEmp.OKWG, PSKTran2.recwg, V_JobMaterial_Sum.MatRecWg, JobBills.RtWg
                                        ) 
