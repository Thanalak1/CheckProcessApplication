    ...
    public partial class Foundry : Form
    {
        ...
        private void previewBtn_Click(object sender, EventArgs e)
        {
            ...
            Center.cmd.CommandText = $"SELECT EmpCode, EmpName, Inv_No, DocNo, JobBarCode, JobDate, TypeOfWork, TypeOfBill, Late, TtQy, CASE WHEN Late = 'YES' OR TtQy = 'YES' THEN (PriceJob * FLOOR(OKQty)) / 2 ELSE 0 END AS DeductLate, JobCenter, Wage, DIFFQty, DIFFOrder, DIFFModel, DIFFSI, DIFFWG1, DIFFWG2, DIFFWG3, DIFFMatWG, DIFFTotalWG, DIFFTotalWGPercent, TotalWage" +
                ...
                $", CASE WHEN DATEDIFF(DAY, DATEADD(DAY, (CASE WHEN JobDetail.ChkModel = 1 AND JobDetail.Model > 0 THEN 13 ELSE 10 END), JobHead.JobDate), JobKeeps.mdate) > 10 THEN 'YES' ELSE '' END AS Late" +
                ...
                $", CASE WHEN ISNULL(AccEmp.EmpAmount, 0) + ISNULL(AccEmp.ISAAmount, 0) <> (ISNULL(JobDetail.PriceJob, 0) * FLOOR(ISNULL(AccEmp2.OKQty, 0)) - (ISNULL(AccEmp.DMWG, 0) + ISNULL(AccEmp.LSWG, 0) + ISNULL(AccEmp.DMGemAmount, 0))) THEN 'ค่าแรงไม่ถูกต้อง' ELSE '' END AS Wage" +
                $", CASE WHEN ISNULL(Jobmodel_purchase.qty, 0) <> 0 THEN ISNULL(Jobmodel_purchase.qty, 0) - (ISNULL(AccEmp.OKQTY, 0) / {value}) ELSE 0 END AS DIFFQty, 0 AS DIFF2" +
                $", CASE WHEN ISNULL(Jobmodel_purchase.qty, 0) = 0 THEN 0 ELSE CASE WHEN ISNULL(AccEmp.ModelNewQ, 0) + ISNULL(AccEmp.ModelNewQ2, 0) + ISNULL(AccEmp.ModelNewQ3, 0) >= ISNULL(Jobmodel_purchase.qty, 0) THEN CASE WHEN ISNULL(Jobmodel_purchase.qty, 0) <= CEILING(ISNULL(AccEmp.OKQTY, 0) / {value}) THEN ISNULL(Jobmodel_purchase.qty, 0) ELSE CEILING(ISNULL(AccEmp.OKQTY, 0) / {value}) END ELSE CASE WHEN ISNULL(Jobmodel_purchase.qty, 0) <= CEILING(ISNULL(AccEmp.OKQTY, 0) / {value}) THEN ISNULL(AccEmp.ModelNewQ, 0) + ISNULL(AccEmp.ModelNewQ2, 0) + ISNULL(AccEmp.ModelNewQ3, 0) ELSE CEILING(ISNULL(AccEmp.OKQTY, 0) / {value}) END END END AS DIFFOrder, (ISNULL(V_JobModel_Sum.QtyR_REC, 0) + ISNULL(V_JobModel_Sum.QtyM_REC, 0)) - (ISNULL(V_JobModel_Sum.QtyR_ISS, 0) - ISNULL(V_JobModel_Sum.QtyM_ISS, 0)) AS DIFFModel, (ISNULL(AccEmp2.OKWG, 0) * ISNULL(JobDetail.DMPercent, 0)) / 100 AS DIFFSI, CASE WHEN JobHead.ChkReturn = 0 AND JobHead.ChkAccount = 1 THEN ISNULL(JobDetail.Ttlwg, 0) - ISNULL(AccEmp.ISSWG, 0) - ISNULL(V_JobMaterial_Sum.MatWg, 0) + ISNULL(V_JobMaterial_Sum.MatWg2, 0) ELSE 0 END AS DIFFWG1" +
                ...
                $" LEFT JOIN PSKTran2 ON JobDetail.OrderNo = PSKTran2.Orderno AND JobDetail.LotNo = PSKTran2.LotNo AND JobDetail.Barcode = PSKTran2.Barcode) AS s" +
