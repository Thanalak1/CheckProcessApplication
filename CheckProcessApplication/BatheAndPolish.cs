    ...
    public partial class BatheAndPolish : Form
    {
        ...
        private void previewBtn_Click(object sender, EventArgs e)
        {
            ...
            Center.cmd.CommandText = $@"--select * from V_PolishAndBathe {WHERE} order by DocNo, JobBarcode
                                        ...
                                        --CAST(ISNULL(V_JobMaterial_NoSum.MatRecWg, 0) - (ISNULL(JobDetail.MatWg, 0) + ISNULL(JobDetail.MatWg2, 0)) AS DECIMAL(18, 2)) AS DiffWgLoss,
                                        CAST(ROUND(CAST((ISNULL(JobDetail.MatWg, 0) + ISNULL(JobDetail.MatWg2, 0)) / NULLIF(ISNULL(JobDetail.TtQty, 0), 0) AS DECIMAL(18, 4)), 2) - ((ISNULL(JobDetail.MatWg, 0) + ISNULL(JobDetail.MatWg2, 0)) - ISNULL(V_JobMaterial_NoSum.MatRecWg, 0)) AS DECIMAL(18, 2)) AS DiffWgLoss,

                                        --ROUND(CAST((ISNULL(JobDetail.MatWg, 0) + ISNULL(JobDetail.MatWg2, 0)) / NULLIF(ISNULL(JobDetail.TtQty, 0), 0) - (ISNULL(JobDetail.MatWg, 0) + ISNULL(JobDetail.MatWg2, 0) - ISNULL(V_JobMaterial_NoSum.MatRecWg, 0)) AS DECIMAL(18, 0)), 0) AS UnitLoss,
                                        CASE WHEN CAST(ROUND(CAST((ISNULL(JobDetail.MatWg, 0) + ISNULL(JobDetail.MatWg2, 0)) / NULLIF(ISNULL(JobDetail.TtQty, 0), 0) AS DECIMAL(18, 4)), 2) - (ISNULL(JobDetail.MatWg, 0) + ISNULL(JobDetail.MatWg2, 0) - ISNULL(V_JobMaterial_NoSum.MatRecWg, 0)) AS DECIMAL(18, 2)) > 0 THEN 1 ELSE 0 END AS UnitLoss,
