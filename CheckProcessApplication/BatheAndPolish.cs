    ...
    public partial class BatheAndPolish : Form
    {
        ...
        private void previewBtn_Click(object sender, EventArgs e)
        {
            ...
            Center.cmd.CommandText = $@"WITH BathePolishData AS
                                        ...
                                        CASE WHEN SUM(ISNULL(AccEmp2.OKWG, 0)) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) + ISNULL(JobBills.RtWg, 0) + ISNULL(JobBills.Silver, 0) + ISNULL(V_JobMaterial_Sum.MatRecWg, 0) + SUM(((((ISNULL(AccEmp2.OKWG, 0)) * ISNULL(JobDetail.DMPercent, 0)) / 100) * 100) / 100) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) - (SUM(JobDetail.Ttlwg) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) + ISNULL(V_JobMaterial_Sum.MatRecWg, 0) + ISNULL(CTE_JobGemDetail.GemWg, 0)) > SUM(((((ISNULL(AccEmp2.OKWG, 0)) * ISNULL(JobDetail.DMPercent, 0)) / 100) * 100) / 100) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) THEN 'ไม่ถูกต้อง' ELSE '' END AS DiffWg,
                                        ...
                                        CASE WHEN CTE_ChkBrass.ChkBrass = 1 THEN 0 - ISNULL(CTE_JobGemDetail.GemWg, 0) ELSE SUM(ISNULL(AccEmp2.OKWG, 0)) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) + ISNULL(JobBills.RtWg, 0) + ISNULL(JobBills.Silver, 0) + ISNULL(V_JobMaterial_Sum.MatRecWg, 0) + SUM(((((ISNULL(AccEmp2.OKWG, 0)) * ISNULL(JobDetail.DMPercent, 0)) / 100) * 100) / 100) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) - (SUM(JobDetail.Ttlwg) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) + ISNULL(V_JobMaterial_Sum.MatRecWg, 0) + ISNULL(CTE_JobGemDetail.GemWg, 0)) END AS DiffTWg,
                                        CAST(CASE WHEN CTE_ChkBrass.ChkBrass = 1 THEN 0 - ISNULL(CTE_JobGemDetail.GemWg, 0) ELSE CASE WHEN CTE_ChkBrass.ChkBrass = 1 THEN 0 - ISNULL(CTE_JobGemDetail.GemWg, 0) ELSE SUM(ISNULL(AccEmp2.OKWG, 0)) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) + ISNULL(JobBills.RtWg, 0) + ISNULL(JobBills.Silver, 0) + ISNULL(V_JobMaterial_Sum.MatRecWg, 0) + SUM(((((ISNULL(AccEmp2.OKWG, 0)) * ISNULL(JobDetail.DMPercent, 0)) / 100) * 100) / 100) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) - (SUM(JobDetail.Ttlwg) OVER (PARTITION BY AccEmp.DocNo, AccEmp.EmpCode) + ISNULL(V_JobMaterial_Sum.MatRecWg, 0) + ISNULL(CTE_JobGemDetail.GemWg, 0)) END / NULLIF(SUM(ISNULL(JobDetail.TtlWg, 0) + ISNULL(V_JobMaterial_Sum.MatRecWg, 0) + ISNULL(JobGemDetail.JobWg, 0)) OVER (), 0) END * 100 AS DECIMAL(18, 2)) AS RawPerDiffTTWg,
