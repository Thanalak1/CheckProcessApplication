    ...
    public partial class BatheForms : Form
    {
        ...
        private void previewBtn_Click(object sender, EventArgs e)
        {
            ...
            Center.cmd.CommandText = $@"--select * from V_Bathe {WHERE} order by DocNo, JobBarcode
                                        ...
                                        CASE WHEN ISNULL(AccEmp.EmpAmount, 0) + ISNULL(AccEmp.ISAAmount, 0) <> SUM(ISNULL(AccEmp2.AccPrice, 0) * ISNULL(CASE WHEN AccEmp2.OKQty = 0.5 THEN 0.5 ELSE CASE WHEN TEmpProfile.TitleName = 'บริษัท' THEN AccEmp2.OKQty ELSE FLOOR(AccEmp2.OKQty) END END, 0)) OVER (PARTITION BY AccEmp.DocNo) - (ISNULL(AccEmp.DMWG, 0) + ISNULL(AccEmp.LSWG, 0) + ISNULL(AccEmp.LSGemAmount, 0) + ISNULL(AccEmp.DMGemAmount, 0) + ISNULL(DeductAmount.Deduct, 0)) + (ISNULL(Jobmodel_purchase.qty, 0) * ISNULL(AccEmp.ModelNewP, 0)) THEN 'ค่าแรงไม่ถูกต้อง' ELSE '' END AS Wage,
