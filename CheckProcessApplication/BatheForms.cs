...
CASE WHEN (CASE WHEN CTE_ChkBrass.ChkBrass = 1 THEN 0 - ISNULL(CTE_JobGemDetail.GemWg, 0) ELSE (CASE WHEN JobHead.ChkAccountNot = 1 AND AccEmp2.OKWG > JobDetail.Ttlwg THEN JobDetail.Ttlwg ELSE AccEmp2.OKWG END + CASE WHEN Deduct.OKWG = AccEmp2.OKWG THEN ROUND(ISNULL(V_JobMaterial_Sum.MatRecWg, 0), 2, 1) ELSE 0 END + ISNULL(JobBills.RtWg, 0) + (JobBills.Silver + CASE WHEN Deduct.OKWG = AccEmp2.OKWG THEN ROUND((SUM(AccEmp2.OKWG) - ROUND(JobDetail.MatWg + JobDetail.MatWg2, 2, 1)) * (JobDetail.DMPercent / 100), 2, 1) ELSE 0 END)) - (JobDetail.Ttlwg + ROUND(JobDetail.MatWg + JobDetail.MatWg2, 2, 1) + ISNULL(CTE_JobGemDetail.GemWg, 0)) END) > (CASE WHEN CTE_ChkBrass.ChkBrass = 1 THEN 0 ELSE FLOOR((((AccEmp2.OKWG - (JobDetail.MatWg + JobDetail.MatWg2)) * JobDetail.DMPercent) / 100) * 100) / 100 END) THEN 'ไม่ถูกต้อง' ELSE '' END AS DiffWgSI,
